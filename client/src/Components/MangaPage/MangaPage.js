...
export default function Homepage() {
  ...
  const handleSubmit = (e, userId) => {
    ...
    fetch(url, {
      ...
      .then(data => {
        let html = '';
        html += `<h2>${e.target.status.value}</h2><table><thead><tr><th>Cover</th><th>Manga</th><th>Progress</th><th>Score</th></tr></thead><tbody>`
  ...
  return (
    <div>
      ...
      <form onSubmit={event => handleSubmit(event, userId)}>
        ...
        <button type="submit">View your manga list.</button>
      </form>
